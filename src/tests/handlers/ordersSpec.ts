...
describe('Orders Endpoints Test', () => {
    ...
    it('test index() /orders with jwt', async () => {
        const {status} = await request.get('/orders').set('Authorization', `Bearer ${token}`);
        expect(status).toBe(200);
    });

    it('test show() /orders/:id ', async () => {
        const {status} = await request.get('/orders/1');
        expect(status).toBe(401);
    });
    it('test show() /orders/:id with a jwt ', async () => {
        const {status} = await request.get('/orders/1').set('Authorization', `Bearer ${token}`);
        expect(status).toBe(200);
    });

    it('test create() /orders', async () => {
        const {status} = await request.post('/orders').send({
            status: 'test1',
            user_id: 1,
        });
        expect(status).toBe(401);
    ...
    it("test delete() /orders", async () => {
        const {status} = await request.delete('/orders').send({
            id: 1,
        });
        expect(status).toBe(401);
    });
    ...
    it('test update() /orders', async () => {
        const {status} = await request.put('/orders').send({
            id: 1,
            status: 'test2',
            user_id: 1,
        });
